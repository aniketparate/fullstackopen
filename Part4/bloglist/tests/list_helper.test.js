...
describe('author who has the largest amount of likes', () => {
  ...
  test('when list has only one blog, equals to that likes', () => {
    const resultEmptyBlogList = listHelper.mostLikes(listWithOneBlog)
    expect(resultEmptyBlogList).toEqual({
      author: listWithOneBlog[0].author,
  ...
  test('of bigger list is calculated right', () => {
    const resultEmptyBlogList = listHelper.mostLikes(blogs)
    expect(resultEmptyBlogList).toEqual({
      author: 'Edsger W. Dijkstra',
      likes: 17,
    })
